...
export async function generateMetadata(): Promise<Metadata> {
  ...
  if (!data || !data.metadata) {
    ...
    return {
      title: "Produk Peternakan | Centra Biotech Indonesia",
      description:
        "Solusi bioteknologi untuk peternakan. Tingkatkan produktivitas dan kesehatan ternak Anda dengan produk inovatif kami.",
    };
  }
  ...
  const imageUrl = data.headline?.image?.url
    ...
    : `${process.env.NEXT_PUBLIC_SITE_URL || "https://centrabiotech.com"}/images/livestock-default.jpg`;

  // Return metadata dinamis
  return {
    title:
      data.metadata.titleTag || "Produk Peternakan | Centra Biotech Indonesia",
    description:
      data.metadata.metaDesc ||
      "Solusi bioteknologi untuk peternakan dari Centra Biotech Indonesia",
    openGraph: {
      title:
        data.metadata.titleTag ||
        "Produk Peternakan | Centra Biotech Indonesia",
      description:
        data.metadata.metaDesc ||
        "Solusi bioteknologi untuk peternakan dari Centra Biotech Indonesia",
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: data.headline?.title || "Produk Peternakan Centra Biotech",
        },
      ],
      locale: "id_ID",
      type: "website",
    ...
    alternates: {
      canonical: `${process.env.NEXT_PUBLIC_SITE_URL || "https://centrabiotech.com"}/livestock`,
    },
    keywords: [
...
const Livestock = async () => {
  ...
  if (!data) {
    // Handle error case
    return <div>Gagal memuat data produk peternakan</div>;
  }
